...
class grade_report_grader extends grade_report {
    ...
    public function get_right_rows($displayaverages) {
        ...
        $headerTheories = getTheoriesForHeader($this->courseid);
        foreach ($headerTheories as $theories) {
            $_moduleType = $theories->name;
            $_moduleTitle = $theories->title;
            $_courseModuleId = $theories->id;
            $headerCell = generateHeaderCell($this->courseid, $_moduleType, $_moduleTitle, $_courseModuleId);
            $rows[1]->cells[] = $headerCell;
        }
        ...
        foreach($rows as $r) {
            ...
            $uid = intval(str_replace('user_', '', $r->id));
            $userTheories = getUserTheories($this->courseid, $uid);

            foreach ($headerTheories as $theories) {
                $moduleKey = $theories->name . '_' . $theories->id;
                $_cell = generateCell($uid, $userTheories->{$moduleKey}, $theories->name, $theories->id);
                $r->cells[] = $_cell;
            }
        }

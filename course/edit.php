...
if ($editform->is_cancelled()) {
    ...
    if (isset($data->saveandsendemail)) {
        if (isset($data->groups_for_notifications)) {
            sendNotificationsToGroups($data->groups_for_notifications, $id);
        }

        if (isset($data->users_for_notifications) && count($data->users_for_notifications) > 0) {
            sendNotificationsToUsers($data->users_for_notifications, $id);
        }
    }
...
function sendNotificationsToGroups($groups_for_notifications, $courseId) {
    global $DB;

    $sendToEmptyGroups = in_array('WITHOUT_GROUPS', $groups_for_notifications);

    $sql = "SELECT g.id AS group_id, u.*
                FROM mdl_user u
    ...
    foreach ($rs as $user) {
        if (in_array($user->group_id, $groups_for_notifications) || $sendToEmptyGroups && empty($user->group_id)) {
            sendEmailNotification($user, $courseId);
        }
    }
    $rs->close();
}

function sendNotificationsToUsers($users_for_notifications, $courseId) {
    global $DB;

    $list = implode(',', $users_for_notifications);
...
function sendEmailNotification($user, $courseid) {
    ...
    $subject = $metadata['mailinglettersubject'];
    $messagetext = '';
    $messagehtml = format_text($metadata['mailingletter'], FORMAT_HTML);

//    logVarDump($messagehtml, $user->id . ' - ' . $user->firstname . ' ' . $user->lastname);

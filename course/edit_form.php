...
class course_edit_form extends moodleform {
    ...
    function definition() {
        ...
        $groups = $this->get_groups($course->id);
        $select1 = $mform->addElement('select', 'groups_for_notifications', 'Делать рассылку для групп:', $groups);
        $select1->setMultiple(true);
//        $select1->setSelected(array_keys($groups));

        $areanames = $this->get_users($course->id);
        $options = array(
            'multiple' => true,
            'noselectionstring' => 'Не выбрано',
        );
        $mform->addElement('autocomplete', 'users_for_notifications', get_string('searcharea', 'search'), $areanames, $options);

        $mform->addElement('submit', 'saveandsendemail', 'Сохранить и отправить');
    ...
    function get_groups($courseId) {
        ...
        $groups = [];
        $groups['WITHOUT_GROUPS'] = 'Без групп';

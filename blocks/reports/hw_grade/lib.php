...
//    logVarDump($sql, '$sql');
//    logVarDump($cxboxCourse , '$cxboxCourse ');
    $records = $DB->get_records_sql($sql, array('user_id'=>$USER->id));
//    logVarDump($records, '$records');
//    $string = implode('-',array_filter($array, 'rempty'))
//    $coursesList = implode(',', array_keys($records));
    $coursesList = implode(',', (array_filter(array_keys($records), 'rempty')));

    return $coursesList;
}

function getTeachersCoursesID($cxboxCourse)
{
    global $DB, $USER;
    ...
    $rs = $DB->get_recordset_sql($sql, array('user_id'=>$USER->id));
    $resp = [];
...
function getHWforTable($courseID, $groupid = 0, $quizid, $coursesIDList, $usersIDList, $showAllHW, $chboxcourse)
{
    ...
    //logVarDump($sql, '1 - sql');
    //logVarDump($coursesList, '2 - $coursesList');
    //logVarDump($quizAttemptsList, '3 - $quizAttemptsList');
    //logVarDump($attemptsList, '4 - $attemptsList');
    //logVarDump($userOnReview, ' $userOnReview');
